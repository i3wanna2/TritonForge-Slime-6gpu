...
# Kernel Code Generation Agent Training — Qwen3-8B-SFT multi-turn (6-GPU + 蹭卡)
...
#   eval reserved               : 7        (+ 蹭卡 2-5 when actor offloaded)
#
# Uses sync train.py + --offload (not train_async) so generate/train do not overlap.
# Protocol: --rm-type / --rollout-task-type kernelbench_multiturn

# Hard cleanup of OUR leftover train processes — never pkill -9 python.
...
# Separate SAVE from single-turn RL so we do not resume iter99 single into multi.
export MCORE_MODEL_PATH_SAVE="${MCORE_MODEL_PATH_SAVE:-${PROJECT_ROOT}/models/Qwen3-8B-Kernelbook-SFT-filtered_save_multiturn}"
# Default: cold from SFT (blog-aligned). Override with TRAIN_MODE=resume if needed.
...
WANDB_ARGS=(
   ...
   --wandb-group TF-Qwen3-8B-SFT-KBench-MultiTurn-6gpu
   --wandb-key ${WANDB_KEY}
)
...
echo "蹭卡: gen=${ROLLOUT_NUM_PROCESS} eval_workers=${EVAL_WORKER_GPUS} reserved=${EVAL_RESERVED_DEVICES} borrowable=${EVAL_BORROWABLE_DEVICES}"
echo "protocol: kernelbench_multiturn | train.py + --offload | SAVE=${MCORE_MODEL_PATH_SAVE}"
echo "===================="
